# rules/dotnet-server/common/file-storage.md
...
2. 对象存储 SDK 必须封装在 Infrastructure 层适配类中，业务代码通过接口调用（如 `IFileStorageService`），禁止直接引用 SDK 内部类型。
3. Bucket 命名规范：`{环境}-{服务名}-{用途}`（如 `prod-order-svc-attachments`）。
...
2. 上传文件名必须重命名为唯一标识（如 GUID + 原始扩展名），禁止使用用户原始文件名存储（防止路径注入和覆盖）。
3. 大文件（> 10MB）必须使用流式上传或分片上传，禁止一次性读入内存。ASP.NET Core 中使用 `IFormFile.CopyToAsync(stream)` 流式处理。
4. 上传接口必须设置超时和请求体大小限制（`MaxRequestBodySize`），超时后取消上传并清理已上传分片。
...
|------|------|------|------------|---------|
| **公开** | 无隐私性、面向所有用户可见的资源 | 商品图片、品牌 Logo、公告附件、营销素材 | 公开读（Public Read） | CDN 直接访问 |
...
3. 公开文件的 CDN 域名必须与业务 API 域名分离。
4. 公开文件上传后返回 CDN 完整 URL，前端直接使用，无需再经服务端中转。
...
2. 大文件下载必须支持断点续传（`Range` 请求），禁止一次性读取全文件到内存。

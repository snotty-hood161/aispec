# Go 服务端 PR 评审清单模板

## 文档目标
1. 用于 Go 服务端 PR 评审，评审人逐项核对，确保代码质量达标。
2. 默认适用 `common` 全量规则，评审前先标注架构类型。

## 使用方式
1. **谁用**：PR 评审人（Reviewer）。
2. **何时用**：每次 Go 服务端 PR 提交评审时。
3. **怎么用**：复制清单到 PR 评审评论中，逐项勾选，未通过项写明阻塞原因。

## 前提
1. 评审前先标注架构类型：`monolith` 或 `microservice`（或混合）。
2. 每项必须给出结果：`[x]` 通过 / `[ ]` 不通过（需写阻塞原因）。
3. 如有例外，必须在 PR 说明中记录：原因、边界、回收时间。

## 优先级说明
1. `P0` 为阻塞项，必须全部通过才可合并。
2. `P1` 为改进项，允许带条件合并，但必须登记技术债与回收计划。
3. 评审结论遵循：任一 `P0` 未通过则 `Request Changes`。

---

## PR 基本信息
- [ ] [P0] 已标注适用 profile：`monolith` / `microservice`
- [ ] [P0] 已说明变更目的、影响范围、回滚方案
- [ ] [P0] 已附关键测试结果与验证方式

## 架构与分层
- [ ] [P0] 依赖方向符合 `transport -> service -> repository`，无反向依赖
- [ ] [P0] `main/bootstrap` 仅做组装，不包含业务逻辑
- [ ] [P0] `transport` 未直接访问数据库/缓存/对象存储客户端
- [ ] [P0] `service` 未直接写 SQL/ORM 细节
- [ ] [P0] `repository` 未承载业务状态机或流程编排

## 组件初始化与生命周期
- [ ] [P0] 组件通过 DI 注入，未在 `handler/service` 直接构造客户端
- [ ] [P0] 无 `init()` 外部连接初始化，无包级全局可变单例
- [ ] [P0] 初始化顺序清晰，关闭顺序反向且可验证
- [ ] [P1] 日志组件优先初始化，初始化失败策略明确（fail-fast/降级）
- [ ] [P0] 提供并验证 `/healthz` 与 `/readyz`（关键依赖故障时不可就绪）

## 配置与环境
- [ ] [P0] 使用 `application.yml + application-<profile>.yml` 分层配置
- [ ] [P0] profile 显式指定且白名单校验（非法值启动失败）
- [ ] [P1] 启动日志明确输出生效 profile
- [ ] [P0] 数据库/Redis/MinIO 等参数全部配置化，无硬编码
- [ ] [P0] 数据库类型明确为 `mysql` 或 `postgresql`

## API 与契约
- [ ] [P0] API 版本与契约（OpenAPI/Proto）已同步更新
- [ ] [P0] 响应结构统一：`code/message/data/request_id/timestamp`
- [ ] [P0] HTTP 状态码语义正确（非成功不返回统一 `200`）
- [ ] [P0] 错误响应文案可控，不泄露内部实现细节
- [ ] [P1] `request_id` 注入、透传与日志关联已验证

## 鉴权与安全
- [ ] [P0] `admin` 与 `user` 认证链路独立，无同中间件分支混用
- [ ] [P0] JWT 配置（算法、密钥来源、过期策略）显式且无硬编码
- [ ] [P0] 高风险操作具备权限校验与审计日志
- [ ] [P0] 敏感信息未出现在日志/响应中

## 数据访问与模型
- [ ] [P0] 常规 CRUD 使用持久化模型（`repository/model`）
- [ ] [P0] 统计/报表多表查询使用临时读模型（`repository/query`）
- [ ] [P0] 临时读模型未用于常规写入路径
- [ ] [P0] SQL 无 `SELECT *`，均为参数化查询
- [ ] [P0] 事务边界在 `service`，`repository` 仅执行数据操作

## 错误处理与可观测性
- [ ] [P0] 系统错误被记录，未原样返回给调用方
- [ ] [P0] 错误映射集中在统一中间件，非散落式实现
- [ ] [P0] `panic` 由 recovery 中间件统一处理
- [ ] [P1] 结构化日志、核心指标（QPS/错误率/P95/P99）与追踪可用
- [ ] [P1] 下游依赖成功率与耗时指标已覆盖

## 测试与发布门禁
- [ ] [P0] 通过 `gofmt`、`go vet`、`golangci-lint`、`go test ./...`
- [ ] [P1] 并发改动执行过 `-race`
- [ ] [P0] 缺陷修复包含回归测试
- [ ] [P1] 验证优雅停机：停接新流量、排空在途请求、超时强退
- [ ] [P0] 涉及 API/配置/数据库变更时文档已同步更新

---

## 结论
- [ ] `Approve`（全部 `P0` 通过）
- [ ] `Request Changes`（存在任一 `P0` 未通过）
- [ ] `Conditional Approve`（`P0` 通过，存在 `P1` 未通过且已登记技术债）

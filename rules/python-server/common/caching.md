# rules/python-server/common/caching.md

## 文档目标
1. 定义 Python 服务端缓存使用规范，涵盖 Redis 分布式缓存与本地进程内缓存。
...
| 类型 | 适用场景 | 推荐方案 |
|------|---------|---------|
| **分布式缓存** | 多实例共享、数据一致性要求高 | Redis（Standalone / Cluster） |
| **本地缓存** | 单实例热点数据、极低延迟、允许短暂不一致 | `cachetools`、`functools.lru_cache`、`aiocache` |

1. 项目必须明确缓存选型，禁止同类场景混用多套缓存方案。
2. 本地缓存仅用于读多写少且允许短暂不一致的场景（如配置项、枚举值、热点查询）。
3. 涉及跨实例一致性的数据必须使用分布式缓存。
4. `lru_cache` 仅用于纯函数缓存，禁止用于缓存有副作用或依赖外部状态的结果。
...
3. 缓存键必须集中定义（如 `app/cache/keys.py`），禁止在业务代码中散写魔法字符串。
...
class CacheKeys:
    ...
    def user_profile(user_id: int) -> str:
        return f"order-svc:user:profile:{user_id}"

    @staticmethod
    def user_orders(user_id: int) -> str:
        return f"order-svc:user:orders:{user_id}"
```
...
BASE_TTL = 300  # 5 分钟
ttl = BASE_TTL + random.randint(-30, 30)  # 4.5~5.5 分钟
...
2. 防护方案（至少实施一种）：
   ...
   - **布隆过滤器**：在缓存层前置布隆过滤器（如 `redis-py` 的 `BF.EXISTS`），快速判断数据是否存在。
...
2. 防护方案（至少实施一种）：
   - **互斥锁**：同一时刻只有一个请求回源查询数据库，其他请求等待结果。推荐使用 Redis 分布式锁（`SET key value NX EX`）。
...
4. 分布式锁场景使用 Redis 锁时，必须设置锁超时且使用唯一标识防止误释放。

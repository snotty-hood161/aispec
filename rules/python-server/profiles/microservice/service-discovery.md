# rules/python-server/profiles/microservice/service-discovery.md
...
4. Python 服务注册推荐使用 `python-consul`（Consul）或直接对接 K8s Service DNS。
...
4. FastAPI 项目的注册/注销逻辑 MUST 放在 `lifespan` 事件中，确保生命周期一致。
...
4. Python 服务 MUST 使用异步健康检查（`async def`），避免阻塞 Worker。
...
@app.get("/readyz")
async def readiness(db: AsyncSession = Depends(get_db_session), redis=Depends(get_redis)):
    errors = []
    try:
    ...
    try:
        ...
        errors.append("redis")
    if errors:
        raise HTTPException(status_code=503, detail={"unavailable": errors})
    return {"status": "ready"}
```

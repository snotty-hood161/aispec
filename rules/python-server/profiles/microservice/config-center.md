# rules/python-server/profiles/microservice/config-center.md
...
4. Python 客户端推荐：`python-consul`（Consul）、`nacos-sdk-python`（Nacos）、`etcd3`（etcd）。
...
4. `pydantic-settings` 管理的配置为启动时加载的静态配置；动态配置需额外的配置中心客户端。
...
class DynamicConfig:
    ...
    def update_rate_limit(self, new_value: int):
        with self._lock:
            old_value = self._rate_limit
            self._rate_limit = new_value
            logger.info("config_updated", key="rate_limit", old=old_value, new=new_value)
```
...
5. Python 项目推荐使用配置中心 KV 存储或专用功能开关服务（如 `Unleash`、`LaunchDarkly`）。

### 功能开关使用示例
```python
async def create_order(request: CreateOrderRequest):
    order = await order_service.create(request)
    if dynamic_config.is_feature_enabled("new_notification_system"):
        await notification_service.send_v2(order)
    else:
        await notification_service.send_v1(order)
    return order
```

# rules/python-server/profiles/microservice/communication-and-contracts.md
...
7. FastAPI 服务的 OpenAPI 文档必须保持与代码同步，CI 中 SHOULD 校验 OpenAPI spec 无漂移。
...
| 服务间通信（Python 生态优先） | **HTTP/JSON** + `httpx` | Python gRPC 工具链相对 Go 较弱时的备选 |

### MUST
1. 微服务间内部通信推荐使用 **gRPC + Proto3**（推荐 `grpcio` + `grpcio-tools`），或 **HTTP/JSON**（推荐 `httpx.AsyncClient`）。
...
### 多语言服务协作（MUST）
1. 跨语言服务间通信必须使用 **gRPC + Proto3** 作为统一协议，禁止依赖语言特定的序列化格式（如 `pickle`）。
2. Proto 文件作为跨语言唯一契约源，由服务提供方维护，消费方通过代码生成获取客户端。
3. 禁止手写跨语言客户端代码，必须通过 Proto 代码生成工具自动生成。
4. Python gRPC 服务推荐使用 `grpcio` + `grpcio-tools`，或 `betterproto` 生成类型安全的异步客户端。

### SHOULD
1. 服务间 HTTP 调用推荐使用 `httpx.AsyncClient`（连接复用），配合 Pydantic schema 进行响应解析。
...
4. 服务间调用必须透传 trace 上下文（`traceparent` Header / gRPC metadata）和请求标识（`request_id`）。
5. 下游不可用时必须有降级策略或显式失败策略，禁止无限等待。
6. 超时设置分层：全局默认 → 服务级 → 接口级，接口级优先。
7. `httpx.AsyncClient` 调用必须设置 `timeout`（含 `connect`、`read`、`write`、`pool`），禁止使用默认无超时。

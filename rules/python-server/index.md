# rules/python-server/index.md

## 目的
1. 统一 Python 服务端开发与交付标准，降低架构漂移和协作成本。
2. 采用"共性规则 + 场景规则"模式，避免重复和冲突。

## 适用范围
1. 适用于所有 Python 服务端代码：FastAPI / Django / Flask HTTP API、后台任务（Celery）、消息消费、定时任务、CLI 工具。
2. 本规则默认高于个人编码习惯；若需例外，必须在评审中记录原因、边界、回收时间。

## 规则组成
1. `common`：所有 Python 服务端必须遵守。
...
3. 混合架构（同仓多服务）：每个可部署单元独立选择 profile，不得混用一套边界定义。

## Skill 协作（推荐）
1. 编写 Python 服务端代码时优先使用 `$python-server-coding-guide`，按编码场景自动加载规则。
2. 跨域业务任务（涉及多个技术栈）使用 `$task-router` 自动分析并路由。
3. Python 服务端规则维护优先使用 `$python-server-rules-maintainer`。
...
15. `common/scheduled-tasks.md` — 定时任务、后台 Worker（Celery）、分布式锁、幂等、失败处理
...
19. `profiles/microservice/communication-and-contracts.md` — 契约治理、协议选型（gRPC/HTTP）、同步调用、数据一致性
20. `profiles/microservice/service-discovery.md` — 服务注册与发现、健康检查、负载均衡
21. `profiles/microservice/resilience.md` — 限流、熔断、降级
22. `profiles/microservice/messaging.md` — 消息队列、异步通信、幂等消费、死信处理
23. `profiles/microservice/gateway.md` — API 网关职责与选型
...
26. `profiles/microservice/config-center.md` — 配置中心选型、动态配置、热更新、功能开关
27. `profiles/microservice/containerization.md` — Docker 镜像规范、K8s 探针与资源限制

### 跨端协作
28. `rules/frontend-backend-collaboration.md` — 前后端契约、联调、发布回滚

### 配套模板 — 参见 `rules/templates/index.md`
- `templates/python-server/pr-review-checklist.md` — Python 服务端 PR 评审清单

# rules/go-server/common/caching.md

## 文档目标
1. 定义 Go 服务端缓存使用规范，涵盖 Redis 分布式缓存与本地进程内缓存。
...
| **本地缓存** | 单实例热点数据、极低延迟、允许短暂不一致 | `sync.Map`、`groupcache`、`ristretto`、`bigcache` |
...
3. 缓存键必须集中定义（如 `internal/cache/keys.go`），禁止在业务代码中散写魔法字符串。
...
2. 防护方案（至少实施一种）：
   - **互斥锁（singleflight）**：同一时刻只有一个请求回源查询数据库，其他请求等待结果。推荐使用 `golang.org/x/sync/singleflight`。

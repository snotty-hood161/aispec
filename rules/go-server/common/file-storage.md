# rules/go-server/common/file-storage.md

## 文档目标
1. 定义文件上传下载、对象存储（MinIO/OSS）使用规范。
2. MinIO 组件初始化参见 `common/component-initialization.md`。
...
2. 对象存储 SDK 必须封装在 `internal/platform/` 适配层中，业务代码通过接口调用，禁止直接引用 SDK 内部类型。
...
2. Bucket 权限必须严格按文件访问分级设置（参见上方分级表），禁止将受限/敏感 Bucket 设为公开读，禁止将任何 Bucket 设为公开写。

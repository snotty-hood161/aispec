# rules/node-server/profiles/microservice/deployment-and-release.md

## Skill 协作
1. `$node-server-coding-guide` 在识别到版本管理、发布流程、灰度发布场景时加载本规则。
...
4. 包版本 MUST 在 `package.json` 中声明，且与 Git tag / Docker 镜像标签一致。
...
1. 数据库迁移（Prisma migrate / TypeORM migration）MUST 在应用部署前独立执行，禁止在应用启动时自动运行。
...
1. CI 流水线必须包含以下阶段：
   - **Lint**：`eslint` + `tsc --noEmit`（类型检查）
   - **Test**：`jest` / `vitest` + 覆盖率检查
   - **Security**：`npm audit` / `pnpm audit` 依赖漏洞扫描
   - **Build**：TypeScript 编译 + Docker 镜像构建 + 标签

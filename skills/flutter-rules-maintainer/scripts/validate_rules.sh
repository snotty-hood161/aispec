...
INDEX_FILE="$RULES_ROOT/flutter/index.md"
FLUTTER_DIR="$RULES_ROOT/flutter"
...
require_dir "$FLUTTER_DIR/common"
require_dir "$FLUTTER_DIR/profiles"
require_dir "$FLUTTER_DIR/profiles/mobile"
...
required_rule_files=(
  ...
  "common/forbidden.md"
  "profiles/mobile/project-structure.md"
)

for rel in "${required_rule_files[@]}"; do
  require_file "$FLUTTER_DIR/$rel"
done
...
for rel_path in "${index_entries[@]}"; do
  full_path="$FLUTTER_DIR/$rel_path"
  if [[ ! -f "$full_path" ]]; then
...
while IFS= read -r line; do
  ...
  find "$FLUTTER_DIR/common" "$FLUTTER_DIR/profiles" -type f -name '*.md' 2>/dev/null \
    | sed "s#^$FLUTTER_DIR/##" \
    | sort
)

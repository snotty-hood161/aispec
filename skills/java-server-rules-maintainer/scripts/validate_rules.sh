...
INDEX_FILE="$RULES_ROOT/java-server/index.md"
COMPAT_FILE="$RULES_ROOT/java-server.md"
JAVA_SERVER_DIR="$RULES_ROOT/java-server"
...
require_dir "$JAVA_SERVER_DIR/common"
require_dir "$JAVA_SERVER_DIR/profiles"
require_dir "$JAVA_SERVER_DIR/profiles/monolith"
require_dir "$JAVA_SERVER_DIR/profiles/microservice"

if ! rg -q 'java-server/index\.md' "$COMPAT_FILE"; then
  fail "兼容入口文件未引用 java-server/index.md"
fi
...
for rel in "${required_rule_files[@]}"; do
  require_file "$JAVA_SERVER_DIR/$rel"
done
...
for rel_path in "${index_entries[@]}"; do
  full_path="$JAVA_SERVER_DIR/$rel_path"
  if [[ ! -f "$full_path" ]]; then
...
while IFS= read -r line; do
  ...
  find "$JAVA_SERVER_DIR/common" "$JAVA_SERVER_DIR/profiles" -type f -name '*.md' 2>/dev/null \
    | sed "s#^$JAVA_SERVER_DIR/##" \
    | sort
)

...
INDEX_FILE="$RULES_ROOT/android/index.md"
COMPAT_FILE="$RULES_ROOT/android.md"
ANDROID_DIR="$RULES_ROOT/android"
...
require_dir "$ANDROID_DIR/common"
require_dir "$ANDROID_DIR/profiles"
require_dir "$ANDROID_DIR/profiles/compose"
require_dir "$ANDROID_DIR/profiles/xml-views"

if ! grep -qF 'android/index.md' "$COMPAT_FILE"; then
  fail "兼容入口文件未引用 android/index.md"
fi
...
required_rule_files=(
  ...
  "common/ui-framework.md"
  "common/forbidden.md"
  "profiles/compose/project-structure.md"
  "profiles/xml-views/project-structure.md"
)

for rel in "${required_rule_files[@]}"; do
  require_file "$ANDROID_DIR/$rel"
done
...
for rel_path in "${index_entries[@]}"; do
  full_path="$ANDROID_DIR/$rel_path"
  if [[ ! -f "$full_path" ]]; then
...
while IFS= read -r line; do
  ...
  find "$ANDROID_DIR/common" "$ANDROID_DIR/profiles" -type f -name '*.md' 2>/dev/null \
    | sed "s#^$ANDROID_DIR/##" \
    | sort
)

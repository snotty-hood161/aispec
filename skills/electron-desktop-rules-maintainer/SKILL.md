---
name: electron-desktop-rules-maintainer
description: 维护并执行 Electron 桌面应用约束规范。用于用户要求新增、修改、重构、审计或对齐 `electron-desktop` 规则体系时触发，适用于 Electron 桌面应用的 IPC 通信、进程安全、自动更新、代码风格等规则维护场景。
...
渲染进程前端代码规范以 `rules/frontend` 为准，本规则仅约束 Electron 主进程与 preload 特有部分。

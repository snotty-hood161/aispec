---
name: java-server-code-reviewer
description: 自动审查 Java 服务端代码变更是否符合规则体系。用于 PR 评审、代码审计、变更合规检查场景；读取 diff/变更文件列表，映射命中规则，逐条检查 MUST/SHOULD 合规性，输出结构化审查报告（P0 阻断 / P1 建议 / 通过项）。
...
- **pr_checklist**: `rules/templates/java-server/pr-review-checklist.md`
...
3. PR 评审清单（参考）：`rules/templates/java-server/pr-review-checklist.md`

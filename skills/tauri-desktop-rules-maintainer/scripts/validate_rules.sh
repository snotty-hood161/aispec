...
INDEX_FILE="$RULES_ROOT/tauri-desktop/index.md"
COMPAT_FILE="$RULES_ROOT/tauri-desktop.md"
TAURI_DIR="$RULES_ROOT/tauri-desktop"
...
require_dir "$TAURI_DIR/common"
require_dir "$TAURI_DIR/profiles"
require_dir "$TAURI_DIR/profiles/tauri-v2"

if ! grep -qF 'tauri-desktop/index.md' "$COMPAT_FILE"; then
  fail "兼容入口文件未引用 tauri-desktop/index.md"
fi
...
required_rule_files=(
  ...
  "common/forbidden.md"
  "profiles/tauri-v2/project-structure.md"
)

for rel in "${required_rule_files[@]}"; do
  require_file "$TAURI_DIR/$rel"
done
...
while IFS= read -r line; do
  ...
  find "$TAURI_DIR/common" "$TAURI_DIR/profiles" -type f -name '*.md' 2>/dev/null \
    | sed "s#^$TAURI_DIR/##" \
    | sort
)
...
echo "通过: tauri-desktop 结构与索引校验通过"

---
name: go-server-project-scaffold
description: 根据部署模式自动初始化 Go 服务端项目结构。用于新项目启动时，输入部署模式（monolith / microservice）后自动读取对应规则，生成目录结构、配置文件、CI 脚本，并输出初始化报告。
...
- **input_type**: 部署模式
- **supported_modes**:
  - `monolith` — 单体应用（HTTP API + 定时任务 + Worker）

...
INDEX_FILE="$RULES_ROOT/ios/index.md"
COMPAT_FILE="$RULES_ROOT/ios.md"
IOS_DIR="$RULES_ROOT/ios"
...
require_dir "$IOS_DIR/common"
require_dir "$IOS_DIR/profiles"
require_dir "$IOS_DIR/profiles/swiftui"
require_dir "$IOS_DIR/profiles/uikit"

if ! grep -qF 'ios/index.md' "$COMPAT_FILE"; then
  fail "兼容入口文件未引用 ios/index.md"
fi
...
required_rule_files=(
  ...
  "common/forbidden.md"
  "profiles/swiftui/project-structure.md"
  "profiles/uikit/project-structure.md"
)

for rel in "${required_rule_files[@]}"; do
  require_file "$IOS_DIR/$rel"
done
...
while IFS= read -r line; do
  ...
  find "$IOS_DIR/common" "$IOS_DIR/profiles" -type f -name '*.md' 2>/dev/null \
    | sed "s#^$IOS_DIR/##" \
    | sort
)

...
INDEX_FILE="$RULES_ROOT/react-native/index.md"
REACT_NATIVE_DIR="$RULES_ROOT/react-native"
...
require_dir "$REACT_NATIVE_DIR/common"
require_dir "$REACT_NATIVE_DIR/profiles"
require_dir "$REACT_NATIVE_DIR/profiles/expo"
require_dir "$REACT_NATIVE_DIR/profiles/bare"
...
required_rule_files=(
  ...
  "common/device-adaptation.md"
  "common/forbidden.md"
  "profiles/expo/project-structure.md"
  "profiles/bare/project-structure.md"
)

for rel in "${required_rule_files[@]}"; do
  require_file "$REACT_NATIVE_DIR/$rel"
done
...
for rel_path in "${index_entries[@]}"; do
  full_path="$REACT_NATIVE_DIR/$rel_path"
  if [[ ! -f "$full_path" ]]; then
...
while IFS= read -r line; do
  ...
  find "$REACT_NATIVE_DIR/common" "$REACT_NATIVE_DIR/profiles" -type f -name '*.md' 2>/dev/null \
    | sed "s#^$REACT_NATIVE_DIR/##" \
    | sort
)
...
echo "通过: react-native 结构与索引校验通过"

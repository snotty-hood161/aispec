...
└── react-native/
    ...
    └── profiles/
        ├── expo/
        │   └── project-structure.md         # Expo managed workflow 项目结构
        └── bare/
            └── project-structure.md         # Bare workflow 项目结构与原生桥接
```
...
2. 索引项必须唯一，禁止重复路径。
3. 索引路径使用相对 `react-native/` 的格式（如 `common/architecture.md`）。

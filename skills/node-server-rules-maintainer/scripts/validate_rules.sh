...
INDEX_FILE="$RULES_ROOT/node-server/index.md"
COMPAT_FILE="$RULES_ROOT/node-server.md"
NODE_SERVER_DIR="$RULES_ROOT/node-server"
...
require_dir "$NODE_SERVER_DIR/common"
require_dir "$NODE_SERVER_DIR/profiles"
require_dir "$NODE_SERVER_DIR/profiles/monolith"
require_dir "$NODE_SERVER_DIR/profiles/microservice"

if ! rg -q 'node-server/index\.md' "$COMPAT_FILE"; then
  fail "兼容入口文件未引用 node-server/index.md"
fi
...
for rel in "${required_rule_files[@]}"; do
  require_file "$NODE_SERVER_DIR/$rel"
done
...
for rel_path in "${index_entries[@]}"; do
  full_path="$NODE_SERVER_DIR/$rel_path"
  if [[ ! -f "$full_path" ]]; then
...
while IFS= read -r line; do
  ...
  find "$NODE_SERVER_DIR/common" "$NODE_SERVER_DIR/profiles" -type f -name '*.md' 2>/dev/null \
    | sed "s#^$NODE_SERVER_DIR/##" \
    | sort
)

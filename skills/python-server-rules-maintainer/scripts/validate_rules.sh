...
INDEX_FILE="$RULES_ROOT/python-server/index.md"
COMPAT_FILE="$RULES_ROOT/python-server.md"
PYTHON_SERVER_DIR="$RULES_ROOT/python-server"
...
require_dir "$PYTHON_SERVER_DIR/common"
require_dir "$PYTHON_SERVER_DIR/profiles"
require_dir "$PYTHON_SERVER_DIR/profiles/monolith"
require_dir "$PYTHON_SERVER_DIR/profiles/microservice"

if ! rg -q 'python-server/index\.md' "$COMPAT_FILE"; then
  fail "兼容入口文件未引用 python-server/index.md"
fi
...
for rel in "${required_rule_files[@]}"; do
  require_file "$PYTHON_SERVER_DIR/$rel"
done
...
for rel_path in "${index_entries[@]}"; do
  full_path="$PYTHON_SERVER_DIR/$rel_path"
  if [[ ! -f "$full_path" ]]; then
...
while IFS= read -r line; do
  ...
  find "$PYTHON_SERVER_DIR/common" "$PYTHON_SERVER_DIR/profiles" -type f -name '*.md' 2>/dev/null \
    | sed "s#^$PYTHON_SERVER_DIR/##" \
    | sort
)
...
echo "通过: python-server 结构与索引校验通过"
